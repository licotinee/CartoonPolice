...
public class CarEnemy_Scene3_4 : MonoBehaviour
{
    ...
    {
        int randomNumber = UnityEngine.Random.Range(94, 96);
        SoundTag soundTag = (SoundTag)Enum.Parse(typeof(SoundTag), randomNumber.ToString());
        Debug.LogError(soundTag);
        ...
        SoundManager.Instance.PlaySFXOneShot(soundTag);
        while (eslapsed <= seconds)
        {
            eslapsed += Time.deltaTime;
            float newValue = 1 - eslapsed / seconds;
