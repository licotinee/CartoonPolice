...
public class WolfooMinigame6 : MonoBehaviour
{
    ...
    IEnumerator StartMoveToStartGame()
    {
        ...
        SoundManager.Instance.PlaySFX(SoundTag.Voice_stop_warning);
        float eslapsed = 0;
        float seconds = 2.5f;
    ...
    private void Jump()
    {
        if (Input.GetMouseButtonDown(0) && !isHitted && GameScene62Manager.ins.isStartGame && !GameScene62Manager.ins.isEndGame &&!isJumpingUp)
        {
            if (CheckOnGround())
            {
                SoundManager.Instance.PlaySFXOneShot(SoundTag.Eff_jump);
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.gameObject.CompareTag("Obsacle"))
        {
            ...
            if (rayCheck)
            {
                if (!isBoosting)
                {
                    ...
                    SoundManager.Instance.PlaySFX(SoundTag.Eff_wolfoo_hit);
        ...
        {
            SoundManager.Instance.PlaySFX(SoundTag.Eff_Wolfoo_roi);
        ...
        {
            SoundManager.Instance.PlaySFX(SoundTag.Eff_Getskin);
    ...
    IEnumerator StartBoost()
    {
        ...
        while (eslapsed <= seconds)
        {
            ...
            if (rayCheck)
            {
                ...
                {
                    SoundManager.Instance.PlaySFX(SoundTag.Eff_object_out);
    ...
    IEnumerator StartMoveToRihino()
    {
        ...
        SoundManager.Instance.PlaySFXOneShot(SoundTag.Eff_wolfoo_ahha);
        eCatchUpWith?.Invoke();
