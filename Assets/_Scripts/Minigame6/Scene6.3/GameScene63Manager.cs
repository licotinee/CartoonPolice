...
public class GameScene63Manager : MonoBehaviour
{
    ...
    {
        ins = this;
        SoundManager.Instance.PlayBGM(SoundTag.Bgm_night_catch);
    ...
    IEnumerator StartScene()
    {
        ...
        eslapsed = 0;
        seconds = 2f;
        SoundManager.Instance.PlaySFXOneShot(SoundTag.Eff_rhino_bike);
        ...
        eslapsed = 0;
        seconds = 1f;
        SoundManager.Instance.PlaySFXOneShot(SoundTag.Eff_night_helicoper);
        while (eslapsed <= seconds)
        {
            eslapsed += Time.deltaTime;
