...
public class CriminalScene72 : Singleton<CriminalScene72>
{
    ...
    {
        base.Awake();
        if (!PlayerPrefs.HasKey(CONSTANTS.FirstPlayScene7_2))
        {
            PlayerPrefs.SetInt(CONSTANTS.FirstPlayScene7_2, 1);
        }
    }
    private void Start()
    {
        ...
        if (PlayerPrefs.GetInt(CONSTANTS.FirstPlayScene7_2) == 1)
        {
            StartCoroutine(nameof(StartSpawnItemTutorial));
            return;
        }
        StartCoroutine(nameof(StartSpawn));
    }

    public void SpawnBannedItem()
    {
        StartCoroutine(nameof(StartSpawn));
    }
    IEnumerator StartSpawnItemTutorial()
    {
        yield return new WaitForSeconds(1f);
        BannedItem itemTutorial = Instantiate(item, posSpawn.transform.position, Quaternion.identity);
        itemTutorial.isItemTutorial = true;
