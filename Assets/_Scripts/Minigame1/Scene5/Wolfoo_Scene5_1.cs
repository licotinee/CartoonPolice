...
public class Wolfoo_Scene5_1 : MonoBehaviour
{
    ...
    bool isMoving;
    float newY;
    private void Move() // check xem vị trí đặt tay để có thể di chuyển
    {
    ...
    IEnumerator StartMove() // di chuyển vị trí theo trục Y
    {
        float eslapsed = 0;
        float seconds = 0.25f;
        ...
        if (PlayerPrefs.GetInt(CONSTANTS.FirstPlayScene1_6) == 1)
        {
            GameScene15Manager.ins.DeActiveTutorial();
        }
